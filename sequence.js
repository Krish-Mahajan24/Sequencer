...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (createPlaylist) createPlaylist.addEventListener('click', () => {
    ...
    const name = (playlistName.value || `Playlist ${loadPersonal('playlists').length + 1}`).trim();
    const playlists = loadPersonal('playlists');
    playlists.push({ id: 'pl_' + Date.now(), name, trackIds: tracks.map(t => t.id), createdAt: new Date().toISOString() });
  ...
  playlistList.addEventListener('click', (e) => {
    const btn = e.target.closest('button[data-action="remove-playlist"]');
    if (!btn) return;
    let playlists = loadPersonal('playlists');
    playlists = playlists.filter(p => String(p.id) !== String(btn.dataset.id));
    savePersonal('playlists', playlists);
    renderCollections();
  });
